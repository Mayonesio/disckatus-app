...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <div class="bg-white rounded-lg shadow-md p-6 mb-6 relative overflow-hidden">
        ...
        <div class="flex flex-col sm:flex-row items-start sm:items-center gap-4 relative z-10">
            ...
            @else
                <div
                    class="w-24 h-24 rounded-full bg-[#10163f] flex items-center justify-center text-white text-3xl border-4 border-white shadow-lg">
                ...
                <div class="mt-2 flex flex-wrap gap-2">
                    @if($user->roles)
                        @foreach($user->roles as $role)
                            <span
                                class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-[#10163f] text-white">
                                {{ $role->name }}
                            </span>
                        @endforeach
                    @endif
            ...
            <div class="flex flex-wrap gap-2">
                @foreach($user->playerProfile->getAllMedals() as $medal)
                    <div class="relative group">
                        <svg class="w-10 h-10" viewBox="0 0 24 24" fill="{{ $medal['color'] }}"
                        ...
                        <span
                            ...
                            {{ substr($medal['name'], 0, 1) }}
                        </span>
                        <div
                            class="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 px-2 py-1 bg-gray-800 text-white text-xs rounded opacity-0 group-hover:opacity-100 transition-opacity duration-300 whitespace-nowrap">
                            {{ $medal['name'] }}: {{ $medal['rating'] }}/10
                        </div>
                    </div>
                @endforeach
            </div>
    ...
    <div class="mt-6 bg-white rounded-lg shadow-md p-6">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
            @foreach($user->playerProfile->getAvailableThrows() as $throw)
                    @if($throw['has_throw'])
                            <div class="flex flex-col items-center space-y-2 p-4 bg-gray-50 rounded-lg">
                                <div class="relative">
                                    <svg class="w-12 h-12" viewBox="0 0 24 24"
                                        fill="{{ \App\Enums\SpecialThrow::from($throw['value'])->getMedalColor($throw['rating']) }}"
                                        xmlns="http://www.w3.org/2000/svg">
                                        <path
                                            d="M12 1L3 5v6c0 5.55 3.84 10.74 9 12 5.16-1.26 9-6.45 9-12V5l-9-4zm0 2.18l7 3.12v5.7c0 4.83-3.36 9.36-7 10.46-3.64-1.1-7-5.63-7-10.46v-5.7l7-3.12z" />
                                        <path fill="#10163f" d="M12 7a5 5 0 100 10 5 5 0 000-10z" />
                                ...
                                @php
                                    $level = \App\Enums\SpecialThrow::from($throw['value'])->getMedalLevel($throw['rating']);
                                @endphp
                                <span class="text-xs px-2 py-1 rounded-full {{ $level['classes'] }}">
                                    {{ $level['text'] }}
                                </span>
                            </div>
                    @endif
        ...
        @if($user->playerProfile->throws_notes)
            <div class="mt-6 p-4 bg-gray-50 rounded-lg">
                <h3 class="text-sm font-medium text-gray-900 mb-2">Observaciones sobre lanzamientos</h3>
